...
<div class="row g-4 mb-4">
    <div class="col-sm-6 col-xl-3">
        <div class="stat-card">
            ...
            <div class="stat-info">
                <h3>{{ $currency }}{{ number_format($totalRevenue, 2) }}</h3>
                <p>Total Revenue</p>
            </div>
        </div>
    </div>
    <div class="col-sm-6 col-xl-3">
        <div class="stat-card">
            ...
            <div class="stat-info">
                <h3>{{ $currency }}{{ number_format($totalCost, 2) }}</h3>
                <p>Total Cost</p>
            </div>
        </div>
    </div>
    <div class="col-sm-6 col-xl-3">
        <div class="stat-card">
            ...
            <div class="stat-info">
                <h3>{{ $currency }}{{ number_format($totalProfit, 2) }}</h3>
                <p>Net Profit</p>
            </div>
        </div>
    </div>
            ...
            <div class="card-body">
                <div class="mb-4">
                    ...
                    <div class="progress mt-2" style="height: 24px;">
                        <div class="progress-bar bg-success" style="width: {{ $totalRevenue > 0 ? ($totalProfit / $totalRevenue) * 100 : 0 }}%">
                            Profit
                        </div>
                        <div class="progress-bar bg-secondary" style="width: {{ $totalRevenue > 0 ? ($totalCost / $totalRevenue) * 100 : 0 }}%">
                            Cost
                        </div>
                    </div>
                </div>
                ...
                <div class="text-center p-3 bg-light rounded">
                    <h5 class="text-muted mb-2">For every {{ $currency }}100 revenue</h5>
                    <div class="row">
                        <div class="col-6">
                            <h4 class="text-success mb-0">{{ $currency }}{{ number_format($profitMargin, 0) }}</h4>
                            <small class="text-muted">Profit</small>
                        </div>
                        <div class="col-6">
                            <h4 class="text-secondary mb-0">{{ $currency }}{{ number_format(100 - $profitMargin, 0) }}</h4>
                ...
                <tbody>
                    @forelse($productProfit as $product)
                        <tr>
                            ...
                            <td class="text-end text-success fw-semibold">{{ $currency }}{{ number_format($product->profit, 2) }}</td>
                            <td class="text-end">
                                <span class="badge bg-{{ $product->revenue > 0 ? (($product->profit / $product->revenue) * 100 > 20 ? 'success' : 'warning') : 'secondary' }}">
                                    {{ $product->revenue > 0 ? number_format(($product->profit / $product->revenue) * 100, 1) : 0 }}%
                                </span>
                            </td>
                        </tr>
                    @empty
...
<script>
    ...
    const dailyProfit = @json($dailyProfit);
