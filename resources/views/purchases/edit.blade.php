...
<div class="row">
    ...
    <div class="col-lg-8">
        <div class="alert alert-info mb-3">
            ...
            Editing <strong>{{ $purchase->reference_number }}</strong>. Changes are saved without affecting stock — stock is only updated when you mark the purchase as <strong>Received</strong>.
        </div>
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table mb-0" id="itemsTable">
                        ...
                        <tbody id="itemsBody">
                            {{-- existing rows rendered by JS from PHP data --}}
                        </tbody>
                    </table>
                </div>
            </div>
                ...
                <div class="row justify-content-end">
                    <div class="col-md-5">
                        <table class="table table-sm mb-0">
                            ...
                            <tr>
                                <td><label class="form-label mb-0 text-muted">Discount</label></td>
                                <td>
                            ...
                            <tr>
                                <td><label class="form-label mb-0 text-muted">Tax</label></td>
                                <td>
                                    <div class="input-group input-group-sm">
                                        <span class="input-group-text">{{ $currency }}</span>
                            ...
                            <tr>
                                <td><label class="form-label mb-0 text-muted">Paid Amount <span class="text-danger">*</span></label></td>
                                <td>
                                    <div class="input-group input-group-sm">
                                        <span class="input-group-text">{{ $currency }}</span>
                    ...
                    <select name="payment_method" class="form-select @error('payment_method') is-invalid @enderror" required>
                        @foreach($paymentMethods as $method)
                            <option value="{{ $method['code'] }}"
                                {{ old('payment_method', $purchase->payment_method) === $method['code'] ? 'selected' : '' }}>
                                {{ $method['name'] }}
                            </option>
                        @endforeach
                    </select>
...
{{-- Product row template (identical to create) --}}
...
// Existing items from the database (pre-encoded in controller)
...
function recalculate() {
    ...
    document.getElementById('summaryTotal').textContent = formatMoney(subtotal - discount + tax);
}

function addItemRow(productId = '', costPrice = '', quantity = 1) {
...
existingItems.forEach(item => addItemRow(item.product_id, item.cost_price, item.quantity));
</script>
@endpush
